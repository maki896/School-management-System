...
import { StudentMarkRecord } from '../data/student-report.service';
    ...
    <mat-card class="panel-card">
      <div class="panel-head">
        <div>
          ...
          <h2>{{ marks.length }} published results</h2>
        </div>
      </div>
      ...
      <article class="record" *ngFor="let mark of marks">
        <div>
          ...
          <p>
            {{ mark.assessmentType }} · {{ mark.term }} · {{ mark.gradeName }}
          </p>
        </div>
...
export class MarksTableComponent {
  @Input({ required: true }) marks: StudentMarkRecord[] = [];
}
